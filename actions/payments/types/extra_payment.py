...
class ExtraPayment(Payment):
    ...
    def apply_to(self, loan: "Loan", advance_payment: bool=False) -> "PaymentEvent":
        """ Apply a payment to a loan object """

        remaining = self.amount_paid

        if advance_payment:
            current_monthly_payment = loan.current_monthly_payment()
            if remaining >= current_monthly_payment:
                n_payments = remaining // current_monthly_payment
                # TODO: Determine how to create future payments (in the loan method?)
            else:
                pass
                # TODO: Determine how to do partial payments
